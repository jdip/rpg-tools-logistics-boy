...
describe('interfaces/ready.ts', { testIsolation: false }, () => {
        ...
        .then(main => {
          ...
          cy.wrap(Object.keys(defs))
            .each((key: keyof typeof defs & string) => {
              cy.get(`#${meta.name}-ready-content input[value="${key}.all"]`)
                .should('be.checked')
                .should('be.visible')
            })
        })
  ...
  describe('Behaves correctly by', () => {
    ...
    it('checking & showing defaults when select-default is clicked', () => {
      cy.waitMainReady()
        .then(main => {
          ...
          cy.get(`#${meta.name}-ready-content a[data-action="select-all"]`)
            .click()
          cy.get(`#${meta.name}-ready-content a[data-action="select-default"]`)
            .click()
            .wrap(Object.keys(defs))
            .each((key: keyof typeof defs & string) => {
              const subgroup = defs[key]
    ...
    it('checking & showing none when select-none is clicked', () => {
      cy.waitMainReady()
        .then(main => {
          ...
          cy.get(`#${meta.name}-ready-content a[data-action="select-all"]`)
            .click()
          cy.get(`#${meta.name}-ready-content a[data-action="select-none"]`)
            .click()
            .wrap(Object.keys(defs))
            .each((key: keyof typeof defs & string) => {
              const subgroup = defs[key]
    ...
    it('enabling/disabling Create Tables button when tables are selected/not selected', () => {
      cy.get(`#${meta.name}-ready-content button[data-action="create-tables"]`)
        .should('not.be.disabled')
        .get(`#${meta.name}-ready-content a[data-action="select-default"]`)
        .click()
        .get(`#${meta.name}-ready-content button[data-action="create-tables"]`)
        .should('not.be.disabled')
        .get(`#${meta.name}-ready-content a[data-action="select-none"]`)
        .click()
        .get(`#${meta.name}-ready-content button[data-action="create-tables"]`)
        .should('be.disabled')
        .get(`#${meta.name}-ready-content a[data-action="select-default"]`)
    ...
    it('enabling/disabling Create Tables button when individual checkbox is clicked', () => {
      cy.get(`#${meta.name}-ready-content button[data-action="create-tables"]`)
        ...
        .get(`#${meta.name}-ready-content button[data-action="create-tables"]`)
        .should('be.disabled')
        .get(`#${meta.name}-ready-content input[type="checkbox"]`)
        .first()
        .click()
        .get(`#${meta.name}-ready-content button[data-action="create-tables"]`)
        .should('not.be.disabled')
        ...
        .then(main => {
          ...
          cy.wrap(Object.keys(defs))
            .each((key: keyof typeof defs & string) => {
              ...
              cy.get(`#${meta.name}-ready-content a[data-group=${key}]`)
                .click()
        ...
        .then(main => {
          ...
          cy.try([`${meta.title}: ${i18n.RTLB.Error.NoDataGroup}`])
            ...
            .then((key: string) => {
              ...
              cy.get<HTMLAnchorElement>(`#${meta.name}-ready-content a[data-group=${key}]`)
                .then($a => {
                  $a.removeData('group')
